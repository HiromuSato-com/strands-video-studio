...
- image_to_clip: 静止画像を指定秒数の動画クリップに変換します
  - duration_sec: クリップの長さ（秒）
  - fps: フレームレート（デフォルト 24）
  - 使用例: スライド風動画 → generate_image → image_to_clip → add_text → crossfade_concat
...
■ Web検索（Tavily MCP）
- tavily-search: リアルタイムWeb検索。参考動画のスタイル・BGMジャンル・プロンプトのアイデアを調べるときに使う
- tavily-extract: 指定URLのページ内容を抽出する
- 使用例: 「ジャズっぽいオープニング動画を作って」→ tavily-searchで参考スタイルを調べてからプロンプト生成

■ AI生成
- generate_video_nova_reel: テキストプロンプトから動画をAIで生成します（Amazon Nova Reel）
- generate_image: テキストプロンプトから画像をAIで生成します（Amazon Nova Canvas）
  - width / height: 画像サイズ（デフォルト 1024x1024、64の倍数、320〜4096）
  - negative_prompt: 画像に含めたくない要素（省略可）
...
記載がない場合 → AI動画生成ツールは使わず、指示された編集作業のみ行うこと
...
- 「スライド風」「発表資料風」「イラスト風スライド」 → generate_image（各スライド背景）→ image_to_clip（静止画→クリップ化）→ add_text（テキスト追加）→ crossfade_concat（トランジション付き結合）
...
def create_agent(video_model: str = "none", extra_tools: list = None) -> Agent:
    model = BedrockModel(
