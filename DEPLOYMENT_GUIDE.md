...
### 5-3. Luma AI Ray 2 の有効化（オレゴン / us-west-2）

1. **右上のリージョンを「米国西部（オレゴン）us-west-2」に切り替える**
2. Bedrock コンソールへの共通手順を実行
3. **「Modify model access」** をクリック
4. **「Luma AI」→「Ray 2」** を探してチェック
5. **「Next」→「Submit」** をクリック
6. S3 バケット作成のダイアログが表示されたら **「確認」または「作成」** をクリック
7. **バケット名を確認してメモ**（例: `bedrock-video-generation-us-west-2-xxxxxx`）
   - 確認方法: AWS コンソール → S3 を開き、`bedrock-video-generation-us-west-2` で始まるバケットを探す
...
### 7-1. variables.tf の編集

`infrastructure/variables.tf` をテキストエディタで開きます。
...
variable "luma_s3_bucket_name" {
  default = "bedrock-video-generation-us-west-2-xxxxxx"  ← 5-3 でメモしたバケット名
}
```

```hcl
variable "nova_reel_s3_bucket_name" {
  default = "bedrock-video-generation-us-east-1-xxxxxx"  ← 5-2 でメモしたバケット名
}
```
...
`bedrock-video-generation-us-west-2-xxxxxx` および `bedrock-video-generation-us-east-1-xxxxxx` は
