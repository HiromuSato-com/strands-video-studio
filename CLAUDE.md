...
# 方法 A（通常）: AWS_PROFILE を設定
export AWS_PROFILE=AWSAdministratorAccess-<account-id>

# 方法 B（A で InvalidClientTokenId が出る場合）: 一時クレデンシャルを環境変数に展開
unset AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_SESSION_TOKEN
eval $(aws configure export-credentials --profile AWSAdministratorAccess-<account-id> --format env)
...
> 古い `AWS_ACCESS_KEY_ID` 等が残っていると `AWS_PROFILE` より優先されて 403 になる。方法 B で確実に解消できる。
...
- AWS CLI / `terraform import` で `/aws/...` や `/ecs/...` 等のパスが Windows パスに変換 → `MSYS_NO_PATHCONV=1` を前置
...
| `terraform apply` で `InvalidClientTokenId` | `export AWS_PROFILE=...` だけでは Terraform の Go SDK が SSO 認証を取れないことがある。古い環境変数をクリアしてから credentials を展開する: `unset AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY AWS_SESSION_TOKEN && eval $(aws configure export-credentials --profile AWSAdministratorAccess-595351378921 --format env) && terraform apply` |
| ECR 削除で `RepositoryNotEmptyException` | Terraform config からリソースを消しただけではイメージが残っているため削除できない。`aws ecr delete-repository --repository-name <name> --region <region> --force` で手動削除してから `terraform apply` を再実行する |
| `terraform apply` で CW Log Group `ResourceAlreadyExistsException` | Lambda が一度でも動くと CW Log Group が自動作成される。Terraform config に追加した場合は `terraform import` が必要: `MSYS_NO_PATHCONV=1 terraform import 'aws_cloudwatch_log_group.lambda["<name>"]' /aws/lambda/<function-name>` |
| Terraform でリソースを削除後に `outputs.tf` 参照エラー | リソース削除時は `outputs.tf` も同時にチェックして参照を除去する |
