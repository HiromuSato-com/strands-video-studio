...
- 動画編集（MoviePy）と AI 動画生成（Amazon Nova Reel）を 1 つの UI から利用できます。
...
| 処理エンジン | Amazon Bedrock AgentCore Runtime + Strands Agents SDK |
| LLM | Amazon Bedrock — Claude Sonnet 4.6 (`us.anthropic.claude-sonnet-4-6`) |
| 動画編集 | MoviePy + ffmpeg (static build) |
| AI 動画生成 | Amazon Nova Reel (`amazon.nova-reel-v1:0`) |
...
| Lambda ×8 | ap-northeast-1 | API ハンドラー + S3 トリガー + runner |
| SQS | ap-northeast-1 | タスクキュー |
...
| ECR | ap-northeast-1 / us-east-1 | コンテナイメージ |
...
| `video_model` | String | `nova_reel` / `none` |
...
| `video_model` | No | string | `nova_reel` / `none`（デフォルト: `none`） |
...
[DynamoDB 書き込み] PENDING
        |
[AgentCore Runtime 起動・main.py 開始]
        |
      RUNNING
        |
  +-----+-----+
...
4. POST /tasks { task_id, instruction, input_keys, video_model: "none" }
   -> DynamoDB: PENDING 書き込み
   -> SQS にメッセージ送信 → runner Lambda → AgentCore Runtime 起動
5. GET /tasks/{id} (ポーリング)
6. AgentCore コンテナ (main.py):
...
### 8.2 AI 動画生成フロー（Amazon Nova Reel）

```
4. POST /tasks { video_model: "nova_reel" }
5. main.py: instruction に "[AI動画生成モデル: Amazon Nova Reel]" を付与
6. Strands Agent:
   a. generate_video_nova_reel(prompt) を呼ぶ
   b. bedrock (us-east-1) で nova-reel-v1:1 を非同期起動
   c. 生成完了まで polling (約 90 秒〜)
   d. bedrock-video-generation-us-east-1-{id}/... からダウンロード
...
| アーキテクチャ | ARM64 |
| ネットワーク | PUBLIC |
| idleRuntimeSessionTimeout | 900 秒 |
...
| AgentCore Runtime タイムアウト | runner Lambda 経由で最大 900 秒（15 分）の同期呼び出し |
| S3 一時ファイル | AgentCore コンテナ内 `/tmp/` を使用（コンテナ終了時に削除） |
